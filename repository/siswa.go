...
func (r *SiswaRepository) GetByEmail(email string) (Siswa, error) {
	var s Siswa
	err := r.db.QueryRow("SELECT * FROM siswa WHERE email = ?", email).Scan(&s.Id, &s.Nama, &s.Password, &s.Email, &s.JenjangPendidikan, &s.Nik, &s.TanggalLahir, &s.TempatLahir)
	if err != nil {
		return s, err
	}
	return s, nil
